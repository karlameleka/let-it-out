...
function randomPrompt(excludeSituation?: string): ReframingPrompt {
  const pool = REFRAMING_PROMPTS.filter(
    (p) => p.situation !== excludeSituation,
  );
...
/** The Cognitive Reframing exercise — a therapist-assigned tool rendered on
 * its own page at /profile/reframing (see MyToolsItem, which links there
 * instead of embedding it), so `onComplete` lets the caller mark the
          ...
          <div className="flex items-center justify-between border-b border-brand-100 bg-brand-50 px-6 py-3 sm:px-8">
            ...
            {count !== null && count > 0 && (
              <p className="text-xs text-ink/40">
                {(count === 1 ? dict.reframeSoFar : dict.reframesSoFar).replace(
                  "{n}",
                  String(count),
                )}
              </p>
          ...
          {step === 0 && (
            <>
              ...
              <p className="mt-3 text-sm text-ink/70">
                {isAr ? prompt.situationAr : prompt.situation}
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mt-0.5 block text-xs text-ink/50">
                        {isAr ? d.descriptionAr : d.description}
                      </span>
                    </button>
                  );
                })}
              ...
              {reframe.trim() && feeling && (
                <div className="mt-5 border-t border-brand-100 pt-5">
                  <p className="text-sm font-medium text-ink/80">
                    {dict.afterFeelingQuestion.replace(
                      "{feeling}",
                      (feelingLabel ?? "").toLowerCase(),
                    )}
                  </p>
                  <IntensityPicker
          ...
          {step === 4 && (
            <div className="animate-pop-in">
              ...
              <p className="mt-1 text-sm text-ink/60">
                {streak !== null &&
                  streak > 1 &&
                  dict.streakLine.replace("{n}", String(streak))}
                {count !== null &&
                  count > 0 &&
                  (count === 1
                    ? dict.reframeSoFar
              ...
              <div className="mt-4 space-y-3 rounded-2xl border border-brand-100 bg-brand-50/60 p-5 text-sm">
                      ...
                      {intensityAfter !== null && (
                        <>
                          ...
                          <span className="font-medium text-brand-700">
                            {intensityLabels[intensityAfter]}
                          </span>{" "}
                          {dict.afterReframing}
                        </>
                      )}
                ...
                {distortions.size > 0 && (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-ink/40">
                      {dict.thinkingTrapsSpottedLabel}
                    </p>
                    <p className="mt-1 text-ink/80">
                      {COGNITIVE_DISTORTIONS.filter((d) =>
                        distortions.has(d.id),
                      )
