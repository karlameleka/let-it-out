...
const SENSES = [
  { count: 5, label: "see", prompt: "Name 5 things you can see around you right now." },
  { count: 4, label: "feel", prompt: "Name 4 things you can physically feel — texture, temperature, weight." },
  { count: 3, label: "hear", prompt: "Name 3 things you can hear right now." },
        ...
        <div className="flex items-center justify-between border-b border-brand-100 bg-brand-50 px-6 py-3 sm:px-8">
          ...
          {count !== null && count > 0 && (
            <p className="text-xs text-ink/40">
              {count} grounding session{count === 1 ? "" : "s"} so far
            </p>
          )}
        </div>
      )}
      ...
      <div className="p-6 sm:p-8">
        {step === 0 && (
          <div className="mb-5 flex items-start gap-2.5 rounded-xl bg-brand-50/70 p-4 text-sm text-ink/70">
            ...
            <p>
              A quick sensory check-in for when your mind is racing — it works by pulling your attention back
              into the present moment, one sense at a time. About 2 minutes, no wrong answers.
            </p>
          </div>
        )}
        ...
        {!done && current && (
          <>
            ...
            <textarea
              ...
              placeholder="Jot down whatever you notice..."
