...
import { updateSessionBooking, type SessionBookingEditFormState } from "@/lib/admin-actions";
...
export default function SessionBookingEditForm({
  booking,
  counselors,
}: {
  booking: {
  ...
  const [state, formAction, pending] = useActionState<SessionBookingEditFormState, FormData>(updateSessionBooking, undefined);
    ...
    <form action={formAction} className="mt-3 w-full space-y-3 rounded-xl border border-brand-100 bg-brand-50/40 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <input name="preferredTime" type="time" defaultValue={booking.preferredTime ?? ""} className={inputClass} />
        </div>
        <div>
