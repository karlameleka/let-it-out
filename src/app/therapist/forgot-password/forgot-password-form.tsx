...
import { forgotCounselorPasswordAction } from "@/lib/therapist-auth-actions";
import { Button } from "@/components/ui";

export default function TherapistForgotPasswordForm() {
  const [state, formAction, pending] = useActionState(forgotCounselorPasswordAction, undefined);

  if (state?.success) {
    return (
      <p className="animate-pop-in text-sm text-ink/70">
        If a portal account exists for that email, we&apos;ve sent a link to reset your password. It
        expires in 1 hour.
      </p>
    );
  }
