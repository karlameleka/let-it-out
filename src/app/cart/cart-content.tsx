...
export default function CartContent({ dict }: { dict: Dictionary["cart"] }) {
  ...
  if (items.length === 0) {
    return (
      <Container className="pt-8 pb-14 text-center sm:pt-14 sm:pb-20">
        <h1 className="font-display text-2xl font-semibold text-brand-900">{dict.emptyTitle}</h1>
        <p className="mt-2 text-ink/60">{dict.emptyBody}</p>
    ...
    <Container className="pt-6 pb-10 sm:pt-14 sm:pb-20">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-brand-100 bg-white p-6 h-fit">
          ...
          <p className="mt-1 text-xs text-ink/50">{dict.shippingNote}</p>
